...
class Palette(BaseModel):
    ...
    def set_input(self, data):
        ...
        self.prev_image = self.set_device(data.get('prev_image'))
        ...
        self.one_hot_seg = self.set_device(data['one_hot_seg'])
        self.cemb = self.set_device(data.get('cemb'))
    ...
    def train_step(self):
        ...
        for train_data in tqdm.tqdm(self.phase_loader):
            ...
            self.optG.zero_grad()
            loss = self.netG(self.gt_image, self.cond_image, mask=self.mask, y_prev=self.prev_image,
                             seg=self.one_hot_seg, cemb=self.cemb)
            loss.backward()
            self.optG.step()
                ...
                if self.opt['distributed']:
                    if self.task in ['inpainting', 'uncropping']:
                        self.output, self.visuals = self.netG.module.restoration(self.cond_image, y_t=self.cond_image,
                                                                                 ...
                                                                                 sample_num=self.sample_num, y_prev=self.prev_image,
                                                                                 slice=self.slice, seg=self.one_hot_seg,
                                                                                 cemb=self.cemb)
                    else:
