...
class Palette(BaseModel):
    ...
    def set_input(self, data):
        ...
        self.ap_mask = self.set_device(data.get('pred_mask'))
    ...
    def train_step(self):
        ...
        for train_data in tqdm.tqdm(self.phase_loader):
            ...
            self.optG.zero_grad()
            loss = self.netG(self.gt_image, self.cond_image, mask=self.mask, ap_mask=self.ap_mask)
            loss.backward()
            self.optG.step()
                ...
                if self.opt['distributed']:
                    if self.task in ['inpainting', 'uncropping']:
                        self.output, self.visuals = self.netG.module.restoration(self.cond_image, y_t=self.cond_image,
                                                                                 ...
                                                                                 sample_num=self.sample_num, ap_mask=self.ap_mask)
                    else:
