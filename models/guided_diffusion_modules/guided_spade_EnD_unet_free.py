...
def gamma_embedding(gammas, dim, max_period=10000):
    ...
    ).to(device=gammas.device) # inner/2
    # print(gammas[:, None].shape, freqs[None].shape) # 3, 1 # 1, 32
...
class EmbedSequential(nn.Sequential, EmbedBlock):
    ...
    def forward(self, x, emb, segmap=None, cemb=None):
        for layer in self:
            if isinstance(layer, EmbedBlock):
                if segmap is not None:
                    x = layer(x, emb, segmap, cemb)
                else:
                    x = layer(x, emb, cemb=cemb)
            else:
                x = layer(x)
        return x
...
class ResBlock(EmbedBlock):
    ...
    def __init__(
        ...
        self.use_cemb = use_cemb
        if use_cemb:
            self.cemb_layers = nn.Sequential(
                SiLU(),
                nn.Linear(
                    emb_channels,
                    2 * self.out_channel if use_scale_shift_norm else self.out_channel,
        ...
        if self.use_spade:
            ...
            self.mlp_shared = nn.Sequential(
                nn.Conv2d(segmap_ch, nhidden, kernel_size=3, padding=1),
                nn.ReLU()
            )
            self.mlp_gamma = nn.Conv2d(nhidden, self.out_channel, kernel_size=3, padding=1)
            self.mlp_beta = nn.Conv2d(nhidden, self.out_channel, kernel_size=3, padding=1)
    ...
    def forward(self, x, emb, segmap=None, cemb=None):
        ...
        """
        return checkpoint(
            self._forward, (x, emb, segmap, cemb), self.parameters(), self.use_checkpoint
        )

    def _forward(self, x, emb, segmap=None, cemb=None):
        if self.updown:
        ...
        if self.use_cemb:
            cemb_out = self.cemb_layers(cemb).type(h.dtype)
            emb_out = emb_out + cemb_out
        ...
        if self.use_scale_shift_norm:
            ...
            h = self.out_layers(h)
        if self.use_spade:
            segmap = F.interpolate(segmap, size=h.size()[2:], mode='nearest')
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        def create_encoder(ds, in_channel, use_spade=False, use_cemb=False):
            ch = input_ch = int(channel_mults[0] * inner_channel)
            input_block_chans = [ch]
            ...
            for level, mult in enumerate(channel_mults):
                for _ in range(res_blocks):
                    ...
                    input_blocks.append(EmbedSequential(*layers))
                    self._feature_size += ch
        ...
        self.input_blocks_x, ch_x, input_block_chans_x = create_encoder(ds, in_channel, True, True)
        self.input_blocks_y, ch_y, input_block_chans_y = create_encoder(ds, guided_img_channel, False)
    ...
    def forward(self, x, gammas, y=None, seg=None, cls=None):
        """
        ...
        # print(cls.shape)
        cemb = self.cls_embed(cls)
        h = x.type(torch.float32)
        for module in self.input_blocks_x:
            h = module(h, emb, seg, cemb)
            hs.append(h)
        h = self.middle_block(h, emb, cemb=cemb)

        ws = []
        w = y.type(torch.float32)
        for module in self.input_blocks_y:
            w = module(w, emb)
            ws.append(w)
        w = self.middle_block(w, emb, cemb=cemb)
...
if __name__ == '__main__':
    ...
    seg = torch.ones((b, 3, h, w)).to(torch.float)
    emb = torch.ones((b, ))
    class_emb = torch.randn((b, 32))
    out = model(x, emb, y, seg, class_emb)
    print(out.shape)

"""
notice must be this file, otherwise error occur. 
SPADE block inside resblock 
only set in decoder
1005
"""