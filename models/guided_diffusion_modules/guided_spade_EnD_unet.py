...
class EmbedSequential(nn.Sequential, EmbedBlock):
    ...
    def forward(self, x, emb, segmap=None):
        for layer in self:
            if isinstance(layer, EmbedBlock):
                if segmap is not None:
                    x = layer(x, emb, segmap)
                else:
                    x = layer(x, emb)
            else:
                x = layer(x)
...
class ResBlock(EmbedBlock):
    ...
    def forward(self, x, emb, segmap=None):
        ...
        """
        return checkpoint(
            self._forward, (x, emb, segmap), self.parameters(), self.use_checkpoint
        )

    def _forward(self, x, emb, segmap=None):
        if self.updown:
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        def create_encoder(ds, in_channel, use_spade=False):
            ch = input_ch = int(channel_mults[0] * inner_channel)
            input_block_chans = [ch]
        ...
        self.input_blocks_x, ch_x, input_block_chans_x = create_encoder(ds, in_channel, True)
    ...
    def forward(self, x, gammas, y=None, seg=None):
        """
        ...
        :return: an [N x C x ...] Tensor of outputs.
        """
        # basic_channel = self.in_channel // 2
        # x = input[:, :basic_channel*2, ::]
        # y = torch.cat([input[:, :basic_channel, ::], input[:, basic_channel*2:, ::]], 1)
        hs = []
...
if __name__ == '__main__':
    ...
    seg = torch.ones((b, 2, h, w)).to(torch.float)
    emb = torch.ones((b, ))
    out = model(x, emb, y, seg)
    print(out.shape)
