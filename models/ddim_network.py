...
class Network(BaseNetwork):
    ...
    @torch.no_grad()
    def make_ddim_sampling_parameters(self, y_t, t, eta=1.0):
        ...
        sigmas = eta * ((1 - alphas_prev) / (1 - alphas)).sqrt() * ((1 - alphas / alphas_prev)).sqrt()
        return sigmas, alphas, alphas_prev

    @torch.no_grad()
    def p_sample(self, y_t, t, clip_denoised=True, y_cond=None, mask=None, y_0=None): # equal to p_sample_ddim
        ...
        if mask is not None:
            y_0_hat = y_0 * (1. - mask) + mask * y_0_hat
        # tmp = y_0_hat.detach().cpu().numpy()
        # tmp = np.concatenate([tmp[i, ::] for i in range(tmp.shape[0])], 2)
        # tmp = ((tmp - tmp.min()) / (tmp.max() - tmp.min()) * 255).astype(np.uint8)
        # import tifffile as tiff
        # tiff.imwrite('tmp.tif', tmp)
        # assert 0
        # if clip_denoised:
        #     y_0_hat.clamp_(-1., 1.)
    ...
    def restoration(self, y_cond, y_t=None, y_0=None, mask=None, sample_num=8):
        b, *_ = y_cond.shape
        self.resampling_timestep_and_parameters()
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            ...
            y_t, pred_y_0 = self.p_sample(y_t, t, y_cond=y_cond, mask=mask, y_0=y_0)
            if mask is not None:
